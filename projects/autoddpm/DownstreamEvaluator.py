...
class PDownstreamEvaluator(DownstreamEvaluator):
    ...
    def __init__(self, name, model, device, test_data_dict, checkpoint_path, global_=True):
        ...
        self.l_pips_sq = lpips.LPIPS(pretrained=True, net='squeeze', use_dropout=True,
                                     eval_mode=True, spatial=True, lpips=True).to(self.device)
    ...
    def _log_visualization(self, to_visualize, i, count):
        ...
        """
        diffp, axarr = plt.subplots(1, len(to_visualize), gridspec_kw={
                                    'wspace': 0, 'hspace': 0}, figsize=(len(to_visualize) * 4, 4))
        for idx, dict in enumerate(to_visualize):
            if 'title' in dict:
                axarr[idx].set_title(dict['title'])
            axarr[idx].axis('off')
            tensor = dict['tensor'][i].cpu().detach().numpy().squeeze() if isinstance(
                dict['tensor'], torch.Tensor) else dict['tensor'][i].squeeze()
            axarr[idx].imshow(tensor, cmap=dict.get('cmap', 'gray'), vmin=dict.get(
                'vmin', 0), vmax=dict.get('vmax', 1))
        diffp.set_size_inches(len(to_visualize) * 4, 4)

        wandb.log(
            {f'Anomaly_masks/Example_Atlas_{count}': [wandb.Image(diffp, caption="Atlas_" + str(count))]})
    ...
    def dilate_masks(self, masks):
        ...
        for i in range(masks.shape[0]):
            ...
            dilated_mask = torch.from_numpy(dilated_mask).to(
                masks.device).unsqueeze(dim=0)
    ...
    def compute_residual(self, x_rec, x, hist_eq=False):
        ...
        if hist_eq:
            ...
            x_rec_rescale = exposure.equalize_adapthist(
                x_rec.cpu().detach().numpy())
            x_res = np.abs(x_rec_rescale - x_rescale)
        else:
            x_res = np.abs(x_rec.cpu().detach().numpy() -
                           x.cpu().detach().numpy())
    ...
    def lpips_loss(self, anomaly_img, ph_img, retPerLayer=False):
        ...
        loss_lpips = self.l_pips_sq(
            anomaly_img, ph_img, normalize=True, retPerLayer=retPerLayer)
        if retPerLayer:
    ...
    def find_mask_size_thresholds(self, dataset):
        ...
        plt.axvline(lower_tail_threshold, color='r', linestyle='--',
                    label=f'25th Percentile: {lower_tail_threshold}')
        plt.axvline(upper_tail_threshold, color='g', linestyle='--',
        ...
        buf.seek(0)
        wandb.log(
            {"Anomaly/Mask sizes1": [wandb.Image(Image.open(buf), caption="Mask Sizes")]})
    ...
    def pathology_localization(self, global_model, th):
            ...
            for idx, data in enumerate(dataset):
                ...
                if not os.path.exists(os.path.join(os.path.dirname(self.model.image_path), f'rec_{self.model.noise_level_recon}', f'image_{global_counter}.png')):
                    os.makedirs(os.path.join(os.path.dirname(
                        self.model.image_path), f'rec_{self.model.noise_level_recon}'), exist_ok=True)
                    os.makedirs(os.path.join(os.path.dirname(
                        self.model.image_path), f'original_{self.model.noise_level_recon}'), exist_ok=True)
                    x_rec, _ = self.model.sample_from_image(
                        x, noise_level=self.model.noise_level_recon)
                    x_rec = torch.clamp(x_rec, 0, 1)

                for i in range(x.shape[0]):
                    path_to_rec = os.path.join(os.path.dirname(
                        self.model.image_path), f'rec_{self.model.noise_level_recon}', f'image_{global_counter}.png')
                    path_to_image = os.path.join(os.path.dirname(
                        self.model.image_path), f'original_{self.model.noise_level_recon}', f'image_{global_counter}.png')
                ...
                x_res = np.asarray([(x_res[i] / np.percentile(x_res[i], 95))
                                   for i in range(x_res.shape[0])]).clip(0, 1)
                combined_mask_np = lpips_mask * x_res
                combined_mask = torch.Tensor(combined_mask_np).to(self.device)
                
                # 获取combined_mask中的最大值和最小值
                # max_value = torch.max(combined_mask)
                # min_value = torch.min(combined_mask)
                # print("最大值:", max_value.item())
                # print("最小值:", min_value.item())

                combined_mask_binary = torch.where(combined_mask > th, torch.ones_like(
                    combined_mask), torch.zeros_like(combined_mask))
                
                combined_mask_binary_dilated = self.dilate_masks(
                    combined_mask_binary)
                
                mask_in_use = combined_mask_binary_dilated

                to_visualize = [
                    {'title': f'l1/95th perc {x_res.max():.3f}', 'tensor': x_res,
                     'cmap': 'plasma'},
                    {'title': f'lpips {lpips_mask.max():.3f}', 'tensor': lpips_mask,
                     'cmap': 'plasma', 'vmax': .4},
                    {'title': f'combined {combined_mask.max():.3f}',
                     'tensor': combined_mask, 'cmap': 'plasma', 'vmax': .3},
                    {'title': f'combined_binary', 'tensor': combined_mask_binary, },
                    {'title': f'comb_bin_dilated',
                        'tensor': combined_mask_binary_dilated, },
                ]
                threshold_masks.append(np.percentile(combined_mask.cpu().detach(
                ).numpy(), 95).mean())  # approximator for good threshold
                ...
                to_visualize.append(
                    {'title': f'x_masked', 'tensor': x_masked, })
                to_visualize.append(
                    {'title': f'x_rec_masked', 'tensor': x_rec_masked, })
                to_visualize.append(
                    {'title': f'just stitched', 'tensor': x_masked + x_rec_masked, })

                # 2. Start inpainting with reconstructed image and not pure noise
                noise = torch.randn_like(x_rec, device=self.device)
                timesteps = torch.full(
                    [x.shape[0]], self.model.noise_level_inpaint, device=self.device).long()
                inpaint_image = self.model.inference_scheduler.add_noise(
                    original_samples=x_rec, noise=noise, timesteps=timesteps
                )

                # 3. Setup for loop
                timesteps = self.model.inference_scheduler.get_timesteps(
                    self.model.noise_level_inpaint)
                ...
                if os.path.exists(os.path.join(self.model.image_path, f'image_{idx * len(x)}.png')):
                    ...
                    for i in range(x.shape[0]):
                        ...
                        path_to_rec = os.path.join(
                            self.model.image_path, f'image_{count}.png')
                        inpainted = Image.open(path_to_rec).convert('L')
                        x_rec_inpainted[i] = transforms.ToTensor()(
                            inpainted).to('cuda')
                                ...
                                for u in range(num_resample_steps):
                                    ...
                                    if t > 0:
                                        ...
                                        timesteps_prev = torch.full(
                                            [x.shape[0]], t - 1, device=self.device).long()
                                    ...
                                    if t > 0:
                                        timesteps = torch.full(
                                            [x.shape[0]], t, device=self.device).long()
                                        model_output = self.model.unet(
                                            x=inpaint_image, timesteps=timesteps)
                                        inpainted_from_x_rec, _ = self.model.inference_scheduler.step(
                                    ...
                                    if t > 0 and u < (num_resample_steps - 1):
                                        inpaint_image = (
                                            torch.sqrt(
                                                1 - self.model.inference_scheduler.betas[t - 1]) * inpaint_image
                                            + torch.sqrt(self.model.inference_scheduler.betas[t - 1]) * torch.randn_like(
                                                x, device=self.device)
                                        )
                    ...
                    for i in range(x.shape[0]):
                        count = str(idx * len(x) + i)
                        path_to_inpainted = os.path.join(
                            self.model.image_path, f'image_{count}.png')
                ...
                print("95th percentile: ", sum(
                    threshold_masks) / len(threshold_masks))

                # 5. Compute new residual and anomaly maps
                x_res_2 = self.compute_residual(
                    x, final_inpainted_image.clamp(0, 1), hist_eq=False)
                x_lpips_2 = self.lpips_loss(
                    x, final_inpainted_image, retPerLayer=False)

                to_visualize.append(
                    {'title': 'final inpainted image', 'tensor': final_inpainted_image})
                to_visualize.append({'title': f'l1 w/ inp. (now eval) {x_res_2.max():.3f}',
                                    'tensor': x_res_2, 'cmap': 'plasma', 'vmax': .9})
                to_visualize.append({'title': f'lpips w/ inpainted {x_lpips_2.max():.3f}',
                                    'tensor': x_lpips_2, 'cmap': 'plasma', 'vmax': .4})
                to_visualize.append({'title': f'comb. w/ inpainted {(x_lpips_2*x_res_2).max():.3f}',
                                    'tensor': x_lpips_2*x_res_2, 'cmap': 'plasma', 'vmax': .2})
                to_visualize.append({'title': f'comb before*after {(combined_mask_np*x_lpips_2*x_res_2).max():.3f}',
                                    'tensor': combined_mask_np*x_lpips_2*x_res_2, 'cmap': 'plasma', 'vmax': .07})
                ...
                # 创建自定义的保存图像数据集文件夹
                # os.makedirs(os.path.join(os.path.dirname(self.model.image_path),
                #             f'anomaly_pred_mask_{self.model.noise_level_recon}'), exist_ok=True)
                
                os.makedirs(os.path.join(os.path.dirname(self.model.image_path),
                            f'anomaly_pred_{self.model.noise_level_recon}'), exist_ok=True)
                os.makedirs(os.path.join(os.path.dirname(self.model.image_path),
                            f'original_mask_{self.model.noise_level_recon}'), exist_ok=True)

                for i in range(len(x)):

                    # get the desired sizes of anomalies
                    if torch.sum(masks[i][0]) > self.model.threshold_low and torch.sum(masks[i][0]) <= self.model.threshold_high:
                        count = str(idx * len(x) + i)
                        ...
                        res_2_i_np = x_res_2[i][0] * \
                            combined_mask[i][0].cpu().detach().numpy()

                        # 这个就是单张图片的预测分数，只有一个数字
                        ...
                        # -------------------------------------------自定义可视化操作--------------------------------------
                        # 保存原始anomaly pred
                        save_image(torch.tensor(res_2_i_np),
                                   os.path.join(os.path.dirname(self.model.image_path),
                                                f'anomaly_pred_{self.model.noise_level_recon}',
                                                f'image_{i}.png'))

                        print("res_2_i_np.max(): {}".format(res_2_i_np.max()))

                        # # 保存预测的anomaly pred mask
                        # save_image(torch.tensor(res_2_i_np.max()),
                        #            os.path.join(os.path.dirname(self.model.image_path),
                        #                         f'anomaly_pred_mask_{self.model.noise_level_recon}',
                        #                         f'image_{i}.png'))

                        # 保存label
                        save_image(masks[i][0],
                                   os.path.join(os.path.dirname(self.model.image_path),
                                                f'original_mask_{self.model.noise_level_recon}',
                                                f'image_{i}.png'))

                        # Similarity metrics: x_rec vs. x
                        loss_mae = self.criterion_rec(rec_2_i, x_i)
                        test_metrics['MAE'].append(loss_mae.item())
                        loss_lpips = np.squeeze(lpips_alex(
                            x_i.cpu(), rec_2_i.cpu()).detach().numpy())
                        test_metrics['LPIPS'].append(loss_lpips)
                        ssim_ = ssim(rec_2_i.cpu().detach().numpy(),
                                     x_i.cpu().detach().numpy(), data_range=1.)
                        test_metrics['SSIM'].append(ssim_)

                    # use slices without anomalies as "healthy" examples on same domain
                    elif torch.sum(masks[i][0]) <= 1:
                        res_2_i_np_healthy = x_res_2[i][0] * \
                            combined_mask[i][0].cpu().detach().numpy()
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            predictions_all = np.reshape(np.asarray(
                predictions), (len(predictions), -1))  # .flatten()
            labels_all = np.reshape(np.asarray(
                labels), (len(labels), -1))  # .flatten()
            ...
            # 计算得到像素级的auroc
            auroc_, _, _, _ = compute_auroc(predictions_all, labels_all)
            # 计算得到像素级的auprc
            auprc_, _, _, _ = compute_auprc(predictions_all, labels_all)

            logging.info(f'Global AUROC score: {auroc_}')
            ...
            for dice_threshold in ths:
                dice = compute_dice(copy.deepcopy(
                    predictions_all), copy.deepcopy(labels_all), dice_threshold)
