        ...
        for epoch in range(self.training_params['nr_epochs']):
            ...
            for data in self.train_ds:
                ...
                transformed_images = self.transform(
                    images) if self.transform is not None else images
                ...
                with autocast(enabled=True):
                    ...
                    noise = generate_noise(
                        self.model.train_scheduler.noise_type, images, self.model.train_scheduler.num_train_timesteps)

                    # Get model prediction
                    pred = self.model(inputs=transformed_images,
            ...
            if (epoch + 1) % self.val_interval == 0 and epoch > 0:
                self.test(self.model.state_dict(), self.val_ds,
                          'Val', self.optimizer.state_dict(), epoch)
