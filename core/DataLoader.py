...
class DefaultDataset(Dataset):
    ...
    def __init__(self, data_dir, file_type='', label_dir=None, mask_dir=None, target_size=(64, 64), test=False):
        ...
        if 'csv' in data_dir[0]:
            ...
            self.files = sorted(glob.glob(data_dir[0] + file_type))
            # self.files = [glob.glob(data_dir_i + file_type) for data_dir_i in data_dir]

        self.nr_items = len(self.files)

        logging.info('DefaultDataset::init(): Loading {} files from: {}'.format(
        ...
        if label_dir is not None:
            if 'csv' in label_dir[0]:
                ...
                self.label_files = sorted(glob.glob(label_dir[0] + file_type))
...
class DefaultDataLoader(pl.LightningDataModule):
    ...
    def __init__(self, args):
        ...
        self.label_dir = args['label_dir'] if 'label_dir' in akeys else {
            'train': None, 'val': None, 'test': None}
        self.mask_dir = args['mask_dir'] if 'mask_dir' in akeys else {
            'train': None, 'val': None, 'test': None}
        self.target_size = args['target_size'] if 'target_size' in akeys else (
            64, 64)
        ...
        assert type(
            self.data_dir) is dict, 'DefaultDataset::init():  data_dir variable should be a dictionary'
