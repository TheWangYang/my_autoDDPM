...
class DLConfigurator(object):
    ...
    def __init__(self, config_file, log_wandb=False):
        ...
        self.device = torch.device(
            'cuda:' + str(0) if torch.cuda.is_available() else 'cpu') if dev == 'gpu' else 'cpu'
        model_class = import_module(
            self.dl_config['model']['module_name'], self.dl_config['model']['class_name'])
        self.model = model_class(**(self.dl_config['model']['params']))

    def start_training(self, global_model: dict = dict()):
        # Trainer Init
        data = self.load_data(
            self.dl_config['trainer']['data_loader'], train=True)
        trainer_class = import_module(
            self.dl_config['trainer']['module_name'], self.dl_config['trainer']['class_name'])
        ...
        logging.info(
            "[Configurator::test]: ################ Starting testing ################")
        # 实际上test函数中没有代码执行，直接跳过这个步骤
        self.trainer.test(trained_model_state,
                          data.test_dataloader(), task='Test')
        logging.info(
            "[Configurator::test]: ################ Finished testing ################")

        # 默认在train和test结束之后开始执行评估函数
        ...
        for idx, dst_name in enumerate(self.dl_config['downstream_tasks']):
            ...
            if 'params' in dst_config.keys():
                ...
                dst = downstream_class(
                    dst_name, self.model, self.device, data, dst_config['checkpoint_path'])
